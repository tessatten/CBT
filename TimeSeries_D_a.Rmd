...
AmAlF_alpha <- rbind(AmAlF1_alpha_CDR3, AmAlF3_alpha_CDR3, AmAlF6_ALL_alpha_CDR3, AmAlF8_ALL_alpha_CDR3)
AmAlF_alpha$ID <- "AmAlF"
AmAlF_alpha$`CDR3 AA sequence` <- as.character(AmAlF_alpha$`CDR3 AA sequence`)
AmAlF_alpha$Month <- as.numeric(AmAlF_alpha$Month)
names(AmAlF_alpha)[names(AmAlF_alpha) == "CDR3 AA sequence"] <- "CDR3s"
names(AmAlF_alpha)[names(AmAlF_alpha) == "Number found in sample"] <- "Count"
dat=AmAlF_alpha
dat  <- dat[,-5] #remove null column
...
sampleName1 <- paste("D", unique(temp1.mod$Month), unique(temp1.mod$Chain), sep = '-') #makes the sample name
sampleName2 <- paste("D", unique(temp2.mod$Month), unique(temp2.mod$Chain), sep = '-')
sampleName3 <- paste("D", unique(temp3.mod$Month), unique(temp3.mod$Chain), sep = '-') #makes the sample name
sampleName4 <- paste("D", unique(temp4.mod$Month), unique(temp4.mod$Chain), sep = '-')
...
scale_alpha_manual(values=c(0, 1)) +
          ...
          scale_x_continuous(breaks = c(1, 3, 5, 7), labels = c(8, 6, 3, 1))+
          ...
          labs(title = 'Shared Clonotypes: Patient D (alpha chain)', x = 'Month', y = 'Cumulative % of Sample')+
...
ggsave("D_alpha_betical3.pdf", width = 40, height = 47, units = c('cm'))
