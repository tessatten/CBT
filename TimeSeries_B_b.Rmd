...
AAM1_beta_CDR3 <- AAM1_beta_dcr_cdr3_freq
AAM1_beta_CDR3$Month <- "1"
AAM1_beta_CDR3$Chain <- "beta"

AAM2_beta_CDR3 <- AAM2_beta_dcr_cdr3_freq
AAM2_beta_CDR3$Month <- "2"
AAM2_beta_CDR3$Chain <- "beta"


#not sure about this
#will try to join data frames
AAM_beta <- rbind(AAM1_beta_CDR3, AAM2_beta_CDR3)
AAM_beta$ID <- "AAM"
AAM_beta$`CDR3 AA sequence` <- as.character(AAM_beta$`CDR3 AA sequence`)
AAM_beta$Month <- as.numeric(AAM_beta$Month)
names(AAM_beta)[names(AAM_beta) == "CDR3 AA sequence"] <- "CDR3s"
names(AAM_beta)[names(AAM_beta) == "Number found in sample"] <- "Count"
dat=AAM_beta
...
scale_alpha_manual(values=c(0, 1)) +
          ...
          labs(title = 'Shared Clonotypes: Patient B (beta chain)', x = 'Month', y = 'Cumulative % of Sample')+
...
ggsave("B_beta_betical.pdf", width = 40, height = 47, units = c('cm'))
