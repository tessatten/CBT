...
```{r}
#time point 
AKMCORD_alpha_CDR3 <- AKMCORD_alpha_dcr_cdr3_freq
AKMCORD_alpha_CDR3$Month <- "0"
AKMCORD_alpha_CDR3$Chain <- "alpha"

AKM2y_alpha_CDR3 <- AKM2y_alpha_dcr_cdr3_freq
AKM2y_alpha_CDR3$Month <- "2"
AKM2y_alpha_CDR3$Chain <- "alpha"


#not sure about this
#will try to join data frames
AKMy_alpha <- rbind(AKMCORD_alpha_CDR3, AKM2y_alpha_CDR3)
AKMy_alpha$ID <- "AKM"
AKMy_alpha$`CDR3 AA sequence` <- as.character(AKMy_alpha$`CDR3 AA sequence`)
AKMy_alpha$Month <- as.numeric(AKMy_alpha$Month)
names(AKMy_alpha)[names(AKMy_alpha) == "CDR3 AA sequence"] <- "CDR3s"
names(AKMy_alpha)[names(AKMy_alpha) == "Number found in sample"] <- "Count"
dat=AKMy_alpha

AKMCORD_alpha_CDR3$Sample <- "CORD"
AKM2x_alpha_CDR3$Sample <- "2x"
AKM2y_alpha_CDR3$Sample <- "2y"
AKM_alpha <- rbind(AKMCORD_alpha_CDR3, AKM2x_alpha_CDR3, AKM2y_alpha_CDR3)
AKM_alpha$ID <- "AKM"
AKM_alpha$`CDR3 AA sequence` <- as.character(AKM_alpha$`CDR3 AA sequence`)
AKM_alpha$Month <- as.numeric(AKM_alpha$Month)
names(AKM_alpha)[names(AKM_alpha) == "CDR3 AA sequence"] <- "CDR3s"
names(AKM_alpha)[names(AKM_alpha) == "Number found in sample"] <- "Count"
...
scale_alpha_manual(values=c(0, 1)) +
          ...
          labs(title = 'Shared Clonotypes: Patient C (alpha chain)', x = 'Month', y = 'Cumulative % of Sample')+
...
ggsave("Cy_alpha_betical.pdf", width = 30, height = 37, units = c('cm'))
