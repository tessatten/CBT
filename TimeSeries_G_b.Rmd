...
DSF1_beta_CDR3 <- DSF1_beta_dcr_cdr3_freq
DSF1_beta_CDR3$Month <- "1"
DSF1_beta_CDR3$Chain <- "beta"

DSF6_beta_CDR3 <- DSF6_beta_dcr_cdr3_freq
DSF6_beta_CDR3$Month <- "6"
DSF6_beta_CDR3$Chain <- "beta"


#not sure about this
#will try to join data frames
DSF_beta <- rbind(DSF1_beta_CDR3, DSF6_beta_CDR3)
DSF_beta$ID <- "DSF"
DSF_beta$`CDR3 AA sequence` <- as.character(DSF_beta$`CDR3 AA sequence`)
DSF_beta$Month <- as.numeric(DSF_beta$Month)
names(DSF_beta)[names(DSF_beta) == "CDR3 AA sequence"] <- "CDR3s"
names(DSF_beta)[names(DSF_beta) == "Number found in sample"] <- "Count"
dat=DSF_beta
G_beta = DSF_beta
...
scale_alpha_manual(values=c(0, 1)) +
          ...
          labs(title = 'Shared Clonotypes: Patient G (beta chain)', x = 'Month', y = 'Cumulative % of Sample')+
...
ggsave("G_beta_betical.pdf", width = 40, height = 47, units = c('cm'))
