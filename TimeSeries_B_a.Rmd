...
---
#here i am going to look by patient at shared clonotypes, if i can - for AAM.
---
...
AAM1_alpha_CDR3 <- AAM1_alpha_dcr_cdr3_freq
AAM1_alpha_CDR3$Month <- "1"
AAM1_alpha_CDR3$Chain <- "alpha"

AAM2_alpha_CDR3 <- AAM2_alpha_dcr_cdr3_freq
AAM2_alpha_CDR3$Month <- "2"
AAM2_alpha_CDR3$Chain <- "alpha"


#not sure about this
#will try to join data frames
AAM_alpha <- rbind(AAM1_alpha_CDR3, AAM2_alpha_CDR3)
AAM_alpha$ID <- "AAM"
AAM_alpha$`CDR3 AA sequence` <- as.character(AAM_alpha$`CDR3 AA sequence`)
AAM_alpha$Month <- as.numeric(AAM_alpha$Month)
names(AAM_alpha)[names(AAM_alpha) == "CDR3 AA sequence"] <- "CDR3s"
names(AAM_alpha)[names(AAM_alpha) == "Number found in sample"] <- "Count"
dat=AAM_alpha
...
sampleName1 <- paste("B", unique(temp1.mod$Month), unique(temp1.mod$Chain), sep = '-') #makes the sample name
sampleName2 <- paste("B", unique(temp2.mod$Month), unique(temp2.mod$Chain), sep = '-')
...
scale_alpha_manual(values=c(0, 1)) +
          ...
          scale_x_continuous(breaks = c(1, 3), labels = c(2, 1))+
          ...
          labs(title = 'Shared Clonotypes: Patient B (alpha chain)', x = 'Month', y = 'Cumulative % of Sample')+
...
ggsave("B_alpha_betical.pdf", width = 40, height = 47, units = c('cm'))
