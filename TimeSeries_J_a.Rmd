...
---
#here i am going to look by patient at shared clonotypes, if i can - for MLM.
---
...
MLM1_alpha_CDR3 <- MLM1_alpha_dcr_cdr3_freq
MLM1_alpha_CDR3$Month <- "1"
MLM1_alpha_CDR3$Chain <- "alpha"

MLM2_alpha_CDR3 <- M_alpha_dcr_cdr3_freq
MLM2_alpha_CDR3$Month <- "2"
MLM2_alpha_CDR3$Chain <- "alpha"

MLM3_alpha_CDR3 <- MLM3_alpha_dcr_cdr3_freq
MLM3_alpha_CDR3$Month <- "3"
MLM3_alpha_CDR3$Chain <- "alpha"


#not sure about this
#will try to join data frames
MLM_alpha <- rbind(MLM1_alpha_CDR3, MLM3_alpha_CDR3, MLM2_alpha_CDR3)
MLM_alpha$ID <- "MLM"
MLM_alpha$`CDR3 AA sequence` <- as.character(MLM_alpha$`CDR3 AA sequence`)
MLM_alpha$Month <- as.numeric(MLM_alpha$Month)
names(MLM_alpha)[names(MLM_alpha) == "CDR3 AA sequence"] <- "CDR3s"
names(MLM_alpha)[names(MLM_alpha) == "Number found in sample"] <- "Count"
dat=MLM_alpha
...
sampleName1 <- paste("J", unique(temp1.mod$Month), unique(temp1.mod$Chain), sep = '-') #makes the sample name
sampleName2 <- paste("J", unique(temp2.mod$Month), unique(temp2.mod$Chain), sep = '-')
sampleName3 <- paste("J", unique(temp3.mod$Month), unique(temp3.mod$Chain), sep = '-') #makes the sample name
...
scale_alpha_manual(values=c(0, 1)) +
          ...
          labs(title = 'Shared Clonotypes: Patient J (alpha chain)', x = 'Month', y = 'Cumulative % of Sample')+
...
ggsave("J_alpha_betical.pdf", width = 40, height = 47, units = c('cm'))
