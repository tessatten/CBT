...
MLM1_beta_CDR3 <- MLM1_beta_dcr_cdr3_freq
MLM1_beta_CDR3$Month <- "1"
MLM1_beta_CDR3$Chain <- "beta"

MLM2_beta_CDR3 <- M_beta_dcr_cdr3_freq
MLM2_beta_CDR3$Month <- "2"
MLM2_beta_CDR3$Chain <- "beta"

MLM3_beta_CDR3 <- MLM3_beta_dcr_cdr3_freq
MLM3_beta_CDR3$Month <- "3"
MLM3_beta_CDR3$Chain <- "beta"


#not sure about this
#will try to join data frames
MLM_beta <- rbind(MLM1_beta_CDR3, MLM3_beta_CDR3, MLM2_beta_CDR3)
MLM_beta$ID <- "MLM"
MLM_beta$`CDR3 AA sequence` <- as.character(MLM_beta$`CDR3 AA sequence`)
MLM_beta$Month <- as.numeric(MLM_beta$Month)
names(MLM_beta)[names(MLM_beta) == "CDR3 AA sequence"] <- "CDR3s"
names(MLM_beta)[names(MLM_beta) == "Number found in sample"] <- "Count"
dat=MLM_beta
J_beta <- MLM_beta
```
...
scale_alpha_manual(values=c(0, 1)) +
          ...
          labs(title = 'Shared Clonotypes: Patient J (beta chain)', x = 'Month', y = 'Cumulative % of Sample')+
...
ggsave("J_beta_betical.pdf", width = 40, height = 47, units = c('cm'))
