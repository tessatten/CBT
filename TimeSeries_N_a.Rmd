...
---
#here i am going to look by patient at shared clonotypes, if i can - for WGM.
---
...
WGM1_alpha_CDR3 <- WGM1_alpha_dcr_cdr3_freq
WGM1_alpha_CDR3$Month <- "1"
WGM1_alpha_CDR3$Chain <- "alpha"

WGM3_alpha_CDR3 <- WGM3_alpha_dcr_cdr3_freq
WGM3_alpha_CDR3$Month <- "3"
WGM3_alpha_CDR3$Chain <- "alpha"

WGM12_alpha_CDR3 <- WGM12_alpha_dcr_cdr3_freq
WGM12_alpha_CDR3$Month <- "12"
WGM12_alpha_CDR3$Chain <- "alpha"


#not sure about this
#will try to join data frames
WGM_alpha <- rbind(WGM1_alpha_CDR3, WGM3_alpha_CDR3, WGM12_alpha_CDR3)
WGM_alpha$ID <- "WGM"
WGM_alpha$`CDR3 AA sequence` <- as.character(WGM_alpha$`CDR3 AA sequence`)
WGM_alpha$Month <- as.numeric(WGM_alpha$Month)
names(WGM_alpha)[names(WGM_alpha) == "CDR3 AA sequence"] <- "CDR3s"
names(WGM_alpha)[names(WGM_alpha) == "Number found in sample"] <- "Count"
dat=WGM_alpha
...
temp3 <- datWithFillseq[datWithFillseq$Month == "12",] #fourth time point
...
sampleName1 <- paste("N", unique(temp1.mod$Month), unique(temp1.mod$Chain), sep = '-') #makes the sample name
sampleName2 <- paste("N", unique(temp2.mod$Month), unique(temp2.mod$Chain), sep = '-')
sampleName3 <- paste("N", unique(temp3.mod$Month), unique(temp3.mod$Chain), sep = '-') #makes the sample name
...
scale_alpha_manual(values=c(0, 1)) +
          ...
          scale_x_continuous(breaks = c(1, 3, 5), labels = c(12, 3, 1))+
          ...
          labs(title = 'Shared Clonotypes: Patient N (alpha chain)', x = 'Month', y = 'Cumulative % of Sample')+
...
ggsave("N_alpha_betical.pdf", width = 40, height = 47, units = c('cm'))
