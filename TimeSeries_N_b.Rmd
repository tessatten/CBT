...
WGM1_beta_CDR3 <- WGM1_beta_dcr_cdr3_freq
WGM1_beta_CDR3$Month <- "1"
WGM1_beta_CDR3$Chain <- "beta"

WGM3_beta_CDR3 <- WGM3_beta_dcr_cdr3_freq
WGM3_beta_CDR3$Month <- "3"
WGM3_beta_CDR3$Chain <- "beta"

WGM12_beta_CDR3 <- WGM12_beta_dcr_cdr3_freq
WGM12_beta_CDR3$Month <- "12"
WGM12_beta_CDR3$Chain <- "beta"


#not sure about this
#will try to join data frames
WGM_beta <- rbind(WGM1_beta_CDR3, WGM3_beta_CDR3, WGM12_beta_CDR3)
WGM_beta$ID <- "WGM"
WGM_beta$`CDR3 AA sequence` <- as.character(WGM_beta$`CDR3 AA sequence`)
WGM_beta$Month <- as.numeric(WGM_beta$Month)
names(WGM_beta)[names(WGM_beta) == "CDR3 AA sequence"] <- "CDR3s"
names(WGM_beta)[names(WGM_beta) == "Number found in sample"] <- "Count"
dat=WGM_beta
...
scale_alpha_manual(values=c(0, 1)) +
          ...
          labs(title = 'Shared Clonotypes: Patient N (beta chain)', x = 'Month', y = 'Cumulative % of Sample')+
...
ggsave("N_beta_betical.pdf", width = 40, height = 47, units = c('cm'))
