...
EKFCORD_alpha_CDR3 <- EKMCORD_alpha_dcr_cdr3_freq
EKFCORD_alpha_CDR3$Month <- "0"
EKFCORD_alpha_CDR3$Chain <- "alpha"

EKF1_alpha_CDR3 <- EKM1_alpha_dcr_cdr3_freq
EKF1_alpha_CDR3$Month <- "0.75"
EKF1_alpha_CDR3$Chain <- "alpha"


#not sure about this
#will try to join data frames
EKF_alpha <- rbind(EKFCORD_alpha_CDR3, EKF1_alpha_CDR3)
EKF_alpha$ID <- "EKF"
EKF_alpha$`CDR3 AA sequence` <- as.character(EKF_alpha$`CDR3 AA sequence`)
EKF_alpha$Month <- as.numeric(EKF_alpha$Month)
names(EKF_alpha)[names(EKF_alpha) == "CDR3 AA sequence"] <- "CDR3s"
names(EKF_alpha)[names(EKF_alpha) == "Number found in sample"] <- "Count"
dat=EKF_alpha
...
temp2 <- datWithFillseq[datWithFillseq$Month == "0.75",] #t2 second time point
...
yadjst <- 9 # so we can move around the not shared label
sampleName1 <- paste("Q", unique(temp1.mod$Month), unique(temp1.mod$Chain), sep = '-') #makes the sample name
sampleName2 <- paste("Q", unique(temp2.mod$Month), unique(temp2.mod$Chain), sep = '-')
...
scale_alpha_manual(values=c(0, 1)) +
          ...
          scale_x_continuous(breaks = c(1, 3), labels = c(1, "CORD"))+
          ...
          labs(title = 'Shared Clonotypes: Patient Q (alpha chain)', x = 'Month', y = 'Cumulative % of Sample')+
...
ggsave("Q_alpha_betical.pdf", width = 40, height = 47, units = c('cm'))
