...
EKFCORD_beta_CDR3 <- EKMCORD_beta_dcr_cdr3_freq
EKFCORD_beta_CDR3$Month <- "0"
EKFCORD_beta_CDR3$Chain <- "beta"

EKF1_beta_CDR3 <- EKM1_beta_dcr_cdr3_freq
EKF1_beta_CDR3$Month <- "0.75"
EKF1_beta_CDR3$Chain <- "beta"


#not sure about this
#will try to join data frames
EKF_beta <- rbind(EKFCORD_beta_CDR3, EKF1_beta_CDR3)
EKF_beta$ID <- "EKF"
EKF_beta$`CDR3 AA sequence` <- as.character(EKF_beta$`CDR3 AA sequence`)
EKF_beta$Month <- as.numeric(EKF_beta$Month)
names(EKF_beta)[names(EKF_beta) == "CDR3 AA sequence"] <- "CDR3s"
names(EKF_beta)[names(EKF_beta) == "Number found in sample"] <- "Count"
dat=EKF_beta
...
scale_alpha_manual(values=c(0, 1)) +
          ...
          labs(title = 'Shared Clonotypes: Patient Q (beta chain)', x = 'Month', y = 'Cumulative % of Sample')+
...
ggsave("Q_beta_betical.pdf", width = 40, height = 47, units = c('cm'))
