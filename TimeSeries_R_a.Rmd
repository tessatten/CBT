...
YAMCORD_alpha_CDR3 <- YAMCORD_alpha_dcr_cdr3_freq
YAMCORD_alpha_CDR3$Month <- "0"
YAMCORD_alpha_CDR3$Chain <- "alpha"

YAM1_alpha_CDR3 <- YAM1_alpha_dcr_cdr3_freq
YAM1_alpha_CDR3$Month <- "1"
YAM1_alpha_CDR3$Chain <- "alpha"


#not sure about this
#will try to join data frames
YAM_alpha <- rbind(YAMCORD_alpha_CDR3, YAM1_alpha_CDR3)
YAM_alpha$ID <- "YAM"
YAM_alpha$`CDR3 AA sequence` <- as.character(YAM_alpha$`CDR3 AA sequence`)
YAM_alpha$Month <- as.numeric(YAM_alpha$Month)
names(YAM_alpha)[names(YAM_alpha) == "CDR3 AA sequence"] <- "CDR3s"
names(YAM_alpha)[names(YAM_alpha) == "Number found in sample"] <- "Count"
dat=YAM_alpha
...
scale_alpha_manual(values=c(0, 1)) +
          ...
          labs(title = 'Shared Clonotypes: Patient R (alpha chain)', x = 'Month', y = 'Cumulative % of Sample')+
...
ggsave("R_alpha_betical.pdf", width = 40, height = 47, units = c('cm'))
