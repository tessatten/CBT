...
---
#here i am going to look by patient at shared clonotypes, if i can - for DSF.
---
...
DSF1_alpha_CDR3 <- DSF1_alpha_dcr_cdr3_freq
DSF1_alpha_CDR3$Month <- "1"
DSF1_alpha_CDR3$Chain <- "alpha"

DSF6_alpha_CDR3 <- DSF6_alpha_dcr_cdr3_freq
DSF6_alpha_CDR3$Month <- "6"
DSF6_alpha_CDR3$Chain <- "alpha"


#not sure about this
#will try to join data frames
DSF_alpha <- rbind(DSF1_alpha_CDR3, DSF6_alpha_CDR3)
DSF_alpha$ID <- "DSF"
DSF_alpha$`CDR3 AA sequence` <- as.character(DSF_alpha$`CDR3 AA sequence`)
DSF_alpha$Month <- as.numeric(DSF_alpha$Month)
names(DSF_alpha)[names(DSF_alpha) == "CDR3 AA sequence"] <- "CDR3s"
names(DSF_alpha)[names(DSF_alpha) == "Number found in sample"] <- "Count"
dat=DSF_alpha
G_alpha = DSF_alpha
```
...
temp2 <- datWithFillseq[datWithFillseq$Month == "6",] #t2 second time point
...
sampleName1 <- paste("G", unique(temp1.mod$Month), unique(temp1.mod$Chain), sep = '-') #makes the sample name
sampleName2 <- paste("G", unique(temp2.mod$Month), unique(temp2.mod$Chain), sep = '-')
...
scale_alpha_manual(values=c(0, 1)) +
          ...
          scale_x_continuous(breaks = c(1, 3), labels = c(6, 1))+
          ...
          labs(title = 'Shared Clonotypes: Patient G (alpha chain)', x = 'Month', y = 'Cumulative % of Sample')+
...
ggsave("G_alpha_betical.pdf", width = 40, height = 47, units = c('cm'))
