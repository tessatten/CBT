...
AKM2x_alpha_CDR3 <- AKM2x_alpha_dcr_cdr3_freq
AKM2x_alpha_CDR3$Month <- "2"
AKM2x_alpha_CDR3$Chain <- "alpha"


#not sure about this
#will try to join data frames
AKMx_alpha <- rbind(AKMCORD_alpha_CDR3, AKM2x_alpha_CDR3)
AKMx_alpha$ID <- "AKM"
AKMx_alpha$`CDR3 AA sequence` <- as.character(AKMx_alpha$`CDR3 AA sequence`)
AKMx_alpha$Month <- as.numeric(AKMx_alpha$Month)
names(AKMx_alpha)[names(AKMx_alpha) == "CDR3 AA sequence"] <- "CDR3s"
names(AKMx_alpha)[names(AKMx_alpha) == "Number found in sample"] <- "Count"
dat=AKMx_alpha
...
ggsave("Cx_alpha_betical.pdf", width = 30, height = 37, units = c('cm'))
