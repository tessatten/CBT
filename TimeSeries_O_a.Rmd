...
YOM2_alpha_CDR3 <- YOM2_alpha_dcr_cdr3_freq
YOM2_alpha_CDR3$Month <- "2"
YOM2_alpha_CDR3$Chain <- "alpha"

YOM12_alpha_CDR3 <- YOM12_alpha_dcr_cdr3_freq
YOM12_alpha_CDR3$Month <- "12"
YOM12_alpha_CDR3$Chain <- "alpha"


#not sure about this
#will try to join data frames
YOM_alpha <- rbind(YOM2_alpha_CDR3, YOM12_alpha_CDR3)
YOM_alpha$ID <- "YOM"
YOM_alpha$`CDR3 AA sequence` <- as.character(YOM_alpha$`CDR3 AA sequence`)
YOM_alpha$Month <- as.numeric(YOM_alpha$Month)
names(YOM_alpha)[names(YOM_alpha) == "CDR3 AA sequence"] <- "CDR3s"
names(YOM_alpha)[names(YOM_alpha) == "Number found in sample"] <- "Count"
dat=YOM_alpha
...
temp2 <- datWithFillseq[datWithFillseq$Month == "12",] #t2 second time point
...
sampleName1 <- paste("O", unique(temp1.mod$Month), unique(temp1.mod$Chain), sep = '-') #makes the sample name
sampleName2 <- paste("O", unique(temp2.mod$Month), unique(temp2.mod$Chain), sep = '-')
...
scale_alpha_manual(values=c(0, 1)) +
          ...
          scale_x_continuous(breaks = c(1, 3), labels = c(12, 2))+
          ...
          labs(title = 'Shared Clonotypes: Patient O (alpha chain)', x = 'Month', y = 'Cumulative % of Sample')+
...
ggsave("O_alpha_betical.pdf", width = 40, height = 47, units = c('cm'))
