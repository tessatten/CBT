...
YAMCORD_beta_CDR3 <- YAMCORD_beta_dcr_cdr3_freq
YAMCORD_beta_CDR3$Month <- "0"
YAMCORD_beta_CDR3$Chain <- "beta"

YAM1_beta_CDR3 <- YAM1_beta_dcr_cdr3_freq
YAM1_beta_CDR3$Month <- "1"
YAM1_beta_CDR3$Chain <- "beta"


#not sure about this
#will try to join data frames
YAM_beta <- rbind(YAMCORD_beta_CDR3, YAM1_beta_CDR3)
YAM_beta$ID <- "YAM"
YAM_beta$`CDR3 AA sequence` <- as.character(YAM_beta$`CDR3 AA sequence`)
YAM_beta$Month <- as.numeric(YAM_beta$Month)
names(YAM_beta)[names(YAM_beta) == "CDR3 AA sequence"] <- "CDR3s"
names(YAM_beta)[names(YAM_beta) == "Number found in sample"] <- "Count"
dat=YAM_beta
...
temp2 <- datWithFillseq[datWithFillseq$Month == "1",] #t2 second time point
...
sampleName1 <- paste("R", unique(temp1.mod$Month), unique(temp1.mod$Chain), sep = '-') #makes the sample name
sampleName2 <- paste("R", unique(temp2.mod$Month), unique(temp2.mod$Chain), sep = '-')
...
scale_alpha_manual(values=c(0, 1)) +
          ...
          labs(title = 'Shared Clonotypes: Patient R (beta chain)', x = 'Month', y = 'Cumulative % of Sample')+
...
ggsave("R_beta_betical.pdf", width = 40, height = 47, units = c('cm'))
