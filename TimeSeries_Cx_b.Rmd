...
AKM2x_beta_CDR3 <- AKM2x_beta_dcr_cdr3_freq
AKM2x_beta_CDR3$Month <- "2"
AKM2x_beta_CDR3$Chain <- "beta"


#not sure about this
#will try to join data frames
AKMx_beta <- rbind(AKMCORD_beta_CDR3, AKM2x_beta_CDR3)
AKMx_beta$ID <- "AKM"
AKMx_beta$`CDR3 AA sequence` <- as.character(AKMx_beta$`CDR3 AA sequence`)
AKMx_beta$Month <- as.numeric(AKMx_beta$Month)
names(AKMx_beta)[names(AKMx_beta) == "CDR3 AA sequence"] <- "CDR3s"
names(AKMx_beta)[names(AKMx_beta) == "Number found in sample"] <- "Count"
dat=AKMx_beta
...
ggsave("Cx_beta_betical.pdf", width = 30, height = 37, units = c('cm'))
