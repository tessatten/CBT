...
YOM2_beta_CDR3 <- YOM2_beta_dcr_cdr3_freq
YOM2_beta_CDR3$Month <- "2"
YOM2_beta_CDR3$Chain <- "beta"

YOM12_beta_CDR3 <- YOM12_beta_dcr_cdr3_freq
YOM12_beta_CDR3$Month <- "12"
YOM12_beta_CDR3$Chain <- "beta"


#not sure about this
#will try to join data frames
YOM_beta <- rbind(YOM2_beta_CDR3, YOM12_beta_CDR3)
YOM_beta$ID <- "YOM"
YOM_beta$`CDR3 AA sequence` <- as.character(YOM_beta$`CDR3 AA sequence`)
YOM_beta$Month <- as.numeric(YOM_beta$Month)
names(YOM_beta)[names(YOM_beta) == "CDR3 AA sequence"] <- "CDR3s"
names(YOM_beta)[names(YOM_beta) == "Number found in sample"] <- "Count"
dat=YOM_beta
...
scale_alpha_manual(values=c(0, 1)) +
          ...
          labs(title = 'Shared Clonotypes: Patient O (beta chain)', x = 'Month', y = 'Cumulative % of Sample')+
...
ggsave("O_beta_betical.pdf", width = 40, height = 47, units = c('cm'))
